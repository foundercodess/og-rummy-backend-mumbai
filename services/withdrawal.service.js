...
async function createWithdrawal({ userId, amount, bankAccountId, type = 'conventional', phone = null }) {
  ...
  try {
    ...
    if (validated.totalDebit > withdrawable) {
      const error = new Error('Insufficient withdrawable balance (amount + handling fee)');
      error.code = 'INSUFFICIENT_BALANCE';
      throw error;
    }
    ...
    const newWithdrawable = roundCurrency(withdrawable - validated.totalDebit);
    const newTotal = roundCurrency(Number(walletRow.total_balance) - validated.totalDebit);
    await client.query(
      `UPDATE wallets
    ...
    if (wallet) {
      ...
      await client.query(
        ...
        [
          ...
          totalDebit,
          withdrawalId,
          JSON.stringify({ reason: reason || 'Payout failed', refunded: true }),
        ]
      );
    }

    const payoutPayload = mergePayoutResponse(tx.payout_response, {
      stage: newStatus === 'rejected' ? 'admin_rejected' : 'pg_init_failed',
      reason,
      gatewayResponse,
      admin_id: adminId || null,
    });
