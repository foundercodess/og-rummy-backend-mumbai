-- FAQs for config API (support, help, etc.)
...
COMMENT ON TABLE faqs IS 'Frequently asked questions returned in config API';
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM faqs LIMIT 1) THEN
    ...
    VALUES
      ('How do I add cash to my account?', 'Go to Wallet > Add Cash and choose an amount. You can pay via UPI, card, or net banking. Minimum add cash is ₹100.', true, 1),
      ('How do I withdraw my winnings?', 'Go to Wallet > Withdraw, select an amount, and complete KYC if required. Withdrawals are processed within 24–48 hours.', true, 2),
